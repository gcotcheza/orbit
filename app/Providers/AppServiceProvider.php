...
final class AppServiceProvider extends ServiceProvider
{
    ...
    {
        /*
         * Fare ports are chosen by name in config/orbit.php; an unknown name
         * throws at resolution rather than silently falling back to a fake.
        ...
        /*
         * Own switch on purpose, though it hits the same vendor as the price
         * port — lets real one-way fares run while returns still use the fake.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Own switch though it reads the same endpoint as the return-trip
         * adapter — the two must be able to fail and be turned off independently.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Bound directly rather than by config name — SerpAPI is the only such
         * adapter. NOT a singleton (holds no state across calls).
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        $this->app->bind(GoogleFlightsCheck::class, function (): GoogleFlightsCheck {
            ...
            return new GoogleFlightsCheck(
                ...
                /*
                 * Empty string reads as unset (same convention as seed.password),
                 * not as a literal key to authenticate a metered API with.
                 * Why: docs/BUSINESS-LOGIC.md §36.
                 */
        ...
        /*
         * Config read once here into a pure value (Domain calls no config()).
         * `max_eur_per_km` × 100 = cents/km, not euros × 100 — check twice.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Own value, not folded into DiscoveryPolicy — two products, not one
         * lane. `min_discount` is a FRACTION (0.40 = 40%), not a euro figure.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        $this->app->singleton(RelativeLanePolicy::class, function (): RelativeLanePolicy {
            ...
            return new RelativeLanePolicy(
                ...
                /*
                 * Same €15 as the absolute lane's, read from the same key —
                 * one decision today, so one setting.
                 * Why: docs/BUSINESS-LOGIC.md §36.
                 */
                minSavingsCents: (int) round(((float) $discovery['min_absolute_savings_eur']) * 100),
        ...
        $this->app->singleton(DealScorer::class, function (): DealScorer {
            ...
            return new DealScorer(new ScoringPolicy(
                ...
                /*
                 * Same number AlertPolicy uses below, on purpose — "too young
                 * to alert" and "too young to score" are one decision.
                 * Why: docs/BUSINESS-LOGIC.md §36.
                 */
        ...
        /*
         * Read once, handed in, same boundary as DealScorer above. Singleton
         * because three consumers share it, not as a perf optimisation.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Read once here, not in Domain — AlertPolicy calls no config(),
         * same boundary as DealScorer/ScoringPolicy above.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Bound directly rather than by config name — mail is the only
         * channel today; web push arrives later as an ADDITION (docs/PLAN.md).
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Chosen by name in config/orbit.php; anthropic COMPOSES regex and
         * falls back to it on any failure. Unknown name throws at resolution.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
    ...
    /**
     * The real fare adapter. Handed scalars rather than reading config()
     * itself, so config:cache and test overrides behave as expected.
     * Missing token throws from the constructor, here, at resolution.
     * Why: docs/BUSINESS-LOGIC.md §36.
     */
    ...
    /**
     * The real round-trip adapter. Shares `orbit.travelpayouts` connection
     * settings; `max_nights`/`limit` are this endpoint's own, in `orbit.returns`.
     * Missing token throws from the constructor, at resolution.
     * Why: docs/BUSINESS-LOGIC.md §36.
     */
    ...
    /**
     * The real origin-sweep adapter. Shares `orbit.travelpayouts` connection
     * settings; `limit` deliberately reads `orbit.returns` too (same endpoint,
     * not a mistake). Missing token throws at resolution.
     * Why: docs/BUSINESS-LOGIC.md §36.
     */
    ...
    /**
     * The statistics Orbit computes for itself. Scalars out of config, like
     * every adapter here — lets a test set the immature end of the blend
     * directly rather than seeding a year of history.
     * Why: docs/BUSINESS-LOGIC.md §36.
     */
    ...
    /**
     * The Claude-backed parser. Transporter is supplied, not discovered:
     * the SDK's own `timeout` is advisory and never read, so only the
     * PSR-18 client's timeout stops a hung request. `http_errors => false`
     * so the SDK's own typed exception wins over a raw Guzzle throw.
     * Why: docs/BUSINESS-LOGIC.md §36.
     */
    ...
    {
        /*
         * NotificationSent fires once a channel returns — the only honest
         * moment for `delivered_at` (see MarkAlertsDelivered).
         * Registered explicitly: no app/Listeners dir exists for discovery.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        Event::listen(NotificationSent::class, MarkAlertsDelivered::class);

        /*
         * Keyed on email AND ip (either alone leaves the other attack free).
         * 5/min: this app has one account, so this route is the whole
         * brute-force surface. Email lower-cased — case is not a bucket.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * Same 5/min as login, on purpose: `current_password` makes this the
         * cheapest way to brute-force an unattended session.
         * Keyed on account not ip — caller is always authenticated here.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * 20/min, keyed on account. Exists before it is needed: the day an
         * Anthropic key lands in .env this becomes a metered call per keystroke.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * 60/min, deliberately generous — guards a CLIENT BUG (debounce
         * failing), not a cost; a limiter a person can trip is worse than
         * what it prevents. No third party or real cost behind this one.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * The one endpoint where a tap spends the fare budget directly.
         * BOTH limits matter: 6/min stops a burst, 20/hour stops a stuck
         * loop from draining the ~200/hour token allowance the daily
         * poll+sweep already uses most of. Keyed on account.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
        ...
        /*
         * DO NOT remove: closes bearer-token auth at the guard. Sanctum falls
         * through to `personal_access_tokens`, a table this app never
         * migrates — left open, a Bearer header turns a 401 into a 500.
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
