 ...
 *
 * Both actions answer the same body — the response is the truth the screen
 * adopts, which is what makes the optimistic switches safe.
 * Why: docs/BUSINESS-LOGIC.md §36.
 *
 * The row is created on first read (UserSettings::for), not by a seeder, so
 * an account that never opened this screen still has settings to read.
 * Why: docs/BUSINESS-LOGIC.md §36.
 */
final class SettingsController extends Controller
{
    ...
    /**
     * ALWAYS 200, PINNED — a JsonResource answers 201 when wasRecentlyCreated,
     * which is wrong here: the row appearing on first read isn't a thing the
     * client made.
     *
     * Why: docs/BUSINESS-LOGIC.md §36.
     */
     ...
     *
     * Sent with every response rather than baked into the screen — a
     * hardcoded "80+" in a template would silently drift from config's
     * `score.tiers`.
     *
     * Why: docs/BUSINESS-LOGIC.md §36.
     *
