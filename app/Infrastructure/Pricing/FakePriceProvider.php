 ...
 * exists, so this is production's adapter until `ORBIT_PRICE_PROVIDER` swaps it.
...
final readonly class FakePriceProvider implements PriceProvider
{
    ...
    public function cheapestPerDay(
        ...
        // Date::now(), not `new DateTimeImmutable`: it's the clock Date::setTestNow() moves,
        // which is how FakeHistorySeeder replays past mornings through the ordinary poller.
        // Why: docs/BUSINESS-LOGIC.md §36.
        ...
        for ($day = $from->setTime(0, 0); $day <= $to; $day = $day->modify('+1 day')) {
            // `foundAt` = this instant: for a simulated fare that's honest (no cache, nothing
            // found it first) — a claim the real adapter usually can't make.
            // Why: docs/BUSINESS-LOGIC.md §36.

            // Not left null: null renders as no freshness line, so the fake would silently hide
            // that feature from every sandbox, e2e run and screenshot.
            // Why: docs/BUSINESS-LOGIC.md §36.
