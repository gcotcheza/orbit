 ...
 *
 * Scheduled daily 04:40 Europe/Amsterdam in routes/console.php (reasoning +
 * collision arithmetic there). Budget: ONE request per watched route per run.
 ...
 *
 * Command, not `Schedule::job()` per route: routes/console.php loads on every
 * artisan invocation (incl. `migrate` on an empty DB) — enumerating routes
...
final class PollReturns extends Command
{
    ...
    public function handle(): int
    {
        ...
        /*
         * Staggered on the same setting the one-way poll uses — no reason
         * for this fan-out to behave differently. Delay is on the QUEUE, so
        ...
        /*
         * NO `--far` flag: the provider answers ~a year regardless, so a
         * narrower window costs the same request for nothing (see PollReturnFares).
         * Why: docs/BUSINESS-LOGIC.md §36.
         */
