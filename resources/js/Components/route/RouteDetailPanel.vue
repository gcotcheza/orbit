...
const pair = computed(() => props.code.toUpperCase())
...
async function load({ quiet = false } = {}) {
  ...
  if (!CODE_PATTERN.test(pair.value)) {
    notFound.value = true
    loading.value = false
  ...
  try {
    const { data } = await http.get(`/api/routes/${pair.value}`)
...
async function lookUp(mine) {
  const [origin, destination] = pair.value.split('-')
...
async function checkLivePrice() {
  ...
  try {
    const { data } = await http.post(
      `/api/routes/${pair.value}/live-price`,
      null,
