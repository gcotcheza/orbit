...
- **"Return trips"**, between the "Check live price" control and the price-history chart. Title Space Grotesk 700 15px, subtitle "Round trip, by length of stay" 12px `--muted`. One row per configured duration band, always all four, in config order: card bg, `--line` hairline, radius 13, 11/13px padding, 8px between rows. Left: the band's name (13.5px 600) over its nights range ("2–3 nights", 10.5px `--muted`). Right, aligned right: "from €112" (Space Grotesk 700 17px, tabular); under it the comparison line in `--ink2` — "8% below its usual €122", which is the price block's own sentence ("Right at its usual €122" when the two agree, which a band of identical fares produces; never "above", because a band's price is the cheapest of the pool its usual price is the median of), or "No usual price yet" in `--muted` where the band is too thin for one; and under that a 10.5px `--muted` meta line, "Sun, Oct 11 · 2 nights", which gains "· seen 4 days ago" only past 24 h — one threshold, shared with the price block's "Seen 3 days ago" line. The date is spelled the way the "Cheapest departure" line spells it rather than the way the mockup drew it ("Sat 11 Oct"): one screen may not hold two date formats.
- **A round-trip fare that may already be gone** takes the "Seen 6 days ago — may be gone" pill (`--warn-ink` on `--warn-bg`, radius 13) above the meta line, and the meta line drops the seen phrase so the age is stated once. Server's judgement, `returns[].fare.mayBeGone`.
- **A band with no fare is a quiet row, not a missing one**: the name in `--muted` 400 and "No return fares seen yet" where the price would be. Sparsity is the ordinary state here, and a band that vanished would read as a feature that had.
- **A row with a fare is one tap target**: the whole card is a link to `returns[].fare.booking.aviasales` (a real `<a>`, opening the way the "See this fare on Aviasales" control opens), with the watchlist row's chevron in `--muted` at the right edge, vertically centred; the card keeps its bg, hairline and radius and shows the app's focus ring when focused by keyboard. A "may be gone" row links too, because Aviasales is exactly where "is it gone?" gets answered. The quiet row is not a link and has no chevron. The touch target is the whole card (≥44px, `docs/STANDARDS.md` T8).
- **The section is drawn wherever the panel is** — the route's own screen, the landing pane and the look-up pane (Ghie, 2026-09-19). In a pane it sits in the left column under the price block; when the detail outgrows the room beside the globe the pane scrolls, and the globe keeps its 280px floor (`docs/DESKTOP-LAYOUT-PLAN.md`). What holds it: the pane test in `resources/js/Components/route/RouteDetailPanel.test.js`, the wide home baselines, and the globe-floor guards in `e2e/specs/layout-screens.spec.js`.
- **A row with a verdict carries the watchlist pill** (`VerdictPill`, size `sm`, `verdict.short` on `verdict.tone`) in the left column under the nights range, 4px below it. A "may be gone" row has no pill, because a fare that may be gone is not scored (`returns[].fare.verdict` is null) — so the two pills never share a row. The pill is the only tinted thing in the row: the price figure, the comparison sentence and the meta line keep their colours (`docs/BUSINESS-LOGIC.md` §9: tone is the only thing to switch colours on).
...
- **Leaving the app**: every control that opens another site in a new tab (the Aviasales and Skyscanner links on Route detail and the day sheet, a priced Return-trips row) ends with the screen-reader-only text ` (opens in a new tab)`, appended inside the link so the spoken name is "See this fare on Aviasales (opens in a new tab)". On top of that, an unmodified click no longer leaves at once: it opens Orbit's own confirmation first — a dialog headed "You're leaving Orbit" that names the destination, with "Continue to {Site}" (a real link, same href, same new tab) beside "Stay in Orbit". It takes the focus, keeps Tab inside itself, closes on Escape or on its scrim and hands the focus back to the link that opened it. A click with ctrl, cmd, shift or alt held, or a middle click, goes straight out with no dialog at all — that choice has already been made.
...
Radii: cards 22, chips/rail 13, calendar cells 9, pills 20, phone screen 38. Spacing: screen gutter 18–20px; type scale 10.5 / 11 / 12 / 13.5 / 15 / 23 / 27px (+ 17px return-row price, + larger detail price).
