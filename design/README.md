...
- **The section is the route's own screen, not the panes.** The landing pane and the look-up pane draw the same panel without it: that pane exists to be a *short* detail beside the globe, and four more rows come straight out of the globe's height (`docs/DESKTOP-LAYOUT-PLAN.md`). What holds it: the pane test in `resources/js/Components/route/RouteDetailPanel.test.js`, and the wide home baselines, which this section must leave untouched.
