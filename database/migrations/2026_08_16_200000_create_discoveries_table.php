 ...
 *
 * The one fare table here deliberately NOT history — a discovery is about
 * a MOMENT, so DiscoverDeals prunes aggressively (~10 rows steady state);
 * a real table, not cache, since an endpoint reads it on the launch path.
 * Why: docs/BUSINESS-LOGIC.md §36.
 *
 * No `route_id`, by design — a discovery is a route nobody's watching, so
 * airports are FK'd but the route is a stored `code` string; the ordinary
 * lookup flow creates the real route only when tapped.
 * Why: docs/BUSINESS-LOGIC.md §36.
 *
 * Three different timestamps — `found_at` (price age), `discovered_at`
 * (sort key), `departure_date` (flight date); see §3 on not mixing them.
 * `expires_at` is stored so the read stays an index range scan, not arithmetic.
 * Why: docs/BUSINESS-LOGIC.md §36.
 */
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('discoveries', function (Blueprint $table): void {
            ...
            // 0-100, or NULL when verification couldn't fetch one — the
            // honest answer is sometimes "we didn't find out".
            // Why: docs/BUSINESS-LOGIC.md §36.
            $table->double('percentile')->nullable();

            // What the finalist saves vs. its window's median, in cents —
            // stored because the window itself is gone by read time.
            // Why: docs/BUSINESS-LOGIC.md §36.
