...
out to this single section, grouped by area and compressed to a sentence or
two each. Every one of them replaced a `// Why: docs/BUSINESS-LOGIC.md §36.`
pointer left at its original site. (`config/orbit.php`, `routes/console.php`,
`routes/web.php`, `app/Providers/AppServiceProvider.php` and
`resources/js/Views/RouteDetail.vue` were slimmed the same way but landed in
their own sections above, or reused existing ones, because their rationale
was substantial enough to deserve real section numbers.)

### Discovery

- **Two-stage funnel.** Sweeping (3 requests, ~1,177 fares) is cheap; verification (~35 requests + ≤5 Google searches, 5 candidates) is what makes a card honest — see §30/§16 for why the sweep's word alone is never enough.
- **One job, not a fan-out.** `DiscoverDeals` ranks across all three origins at once; splitting it per route would mean a rendezvous or three separate, smaller budgets.
- **Idempotent, one try, 900s timeout.** The write is an upsert keyed on `(code, departure_date)`; a retry would re-spend metered requests to reproduce the same screen a day early, so `$tries = 1` and the timeout is generous because nobody is waiting on a 05:20 queue job.
- **Google budget asked for once, up front**, before any of it is spent — a skipped check (no key, quota exhausted) is never an error, just an unverified card.
- **Absolute lane queued before relative.** Both lanes now share one SerpAPI allowance; absolute wins ties because it's the stronger, older claim.
- **Baselines are written before the verification gate**, not after — a candidate that fails verification has still told Orbit what its route costs, which is the more valuable answer over a week (the flywheel).
- **Cross-sectional gate applies unchanged to both lanes** — a relative candidate that skipped it would be making the exact claim the funnel exists to prevent.
- **An empty window disqualifies a relative candidate but not an absolute one.** A relative card's whole claim rests on the fetched window; an absolute card's claim rests on the sweep alone, which coverage gaps (Travelpayouts covers 41-87% even on watched routes) don't undermine.
- **One airport query for the whole sweep** rather than one per destination — 1,177 codes against a 3,270-row table that fits in memory.
- **City/metropolitan codes (LON, MOW, MIL...) are dropped, not resolved** — no coordinates means no honest €/km, and no route code means the card would go nowhere.
- **The relative lane's pool excludes only the absolute lane's five picks**, not everything that cleared the €/km floor — a route that just missed absolute's shortlist is still a fine relative candidate.
- **`rememberBaseline()` is the only Discovery state that outlives a run.** Median and count are stored together (a count-less median can't be trusted); it's an upsert on `code`, so a route has one current belief, not a history.
- **UTC conversion before every upsert**, in both `store()` and `rememberBaseline()` — `upsert()` skips the model's casts, so an unconverted Carbon in the owner's zone is read back as UTC and drifts everything two hours fresher than it is.
- **`lane` and `google_verdict` are both in the upsert's update list**, in both directions — a route that stops clearing the €/km floor must be able to lose its "absolute" badge, and a run without quota must be able to take a verdict away.
- **`prune()` clears superseded same-route rows** from earlier runs (same code, different date) so one route never shows twice.
- **Fourth fare port, the first with no destination in its signature** (`OriginSweepProvider`) — it answers "surprise me," a question that can't be asked about a specific pair.
- **One request per origin covers every destination** — 1,177 candidates for 3 requests, against 1,177 requests to ask route by route.
- **Swept fares are unverified and the port must not make them nicer** — an implementation that sorted, filtered or dropped stale rows would be making the verification stage's decisions with the least information available.
- **`FakeSweepProvider` reads the `airports` table itself** rather than inventing codes, is capped to `MAX_SWEEP_KM = 4000` because `FakeFareModel` is distance-blind (a flat price band under a €/km ranking would rank purely by distance and put Hokitika at the top), and spreads its ages over ~7 days so the freshness rule has something to bite on in the sandbox.
- **`SALE_IN_HUNDREDTHS` (22) is not the funnel's real pass rate (4.9%)** — it's tuned so the shortlist fills in the fake sandbox, a different question from the real funnel's output rate.
- **`GoogleVerdict` reads Google's own market, not a comparison to our price** — the obvious rule ("below Google's typical range") confirmed all three real 2026-08-16 finalists even though Google's own cheapest seat was 2-6x higher; this is the single most important test in the feature.
- **`confirmed` is stored, not recomputed** — a rule retuned next month must not silently restate what last month's badge claimed.
- **Haversine reference figures are Travelpayouts' own `distance` field**, agreeing to 2% on 518/520 destinations — except AMS-BRU, whose returned distance (5951km for a 158km hop) is the regression this test class exists to catch.
- **`DiscoveryBaseline` is the only thing in Discovery that persists across runs**, and must never grow into a `calendar_fares`-shaped cache — that would corrupt the watchlist's notion of a known pair and could feed the rule engine, which sends mail.
- **`Discover` is scheduled (unlike `orbit:poll-returns`) because it ships with a reader on day one** (`GET /api/discoveries`), and it's safe to schedule because nothing here can send mail.
- **`DiscoveryController` is a pure read of a precomputed table**, no parameters, behind `auth:sanctum` like every other read, and uses the owner's clock (not UTC) to match the job that wrote the rows.
- **`RelativePick`'s reason (baseline vs. exploration) travels with the candidate** so a run can be read: most exploration picks fail verification, which is correct and would otherwise look like a bad hit rate.
- **`expectedDiscount()` is a prediction, not the stored result** — what the card prints comes from the freshly fetched window, never from the guess that motivated spending the request.
- **The distance-band baseline design was tried and killed**: `AMS-DUB` at €30/750km scores −3.4% against its own band median (Dublin *is* the median fare for its distance) — the regression that proves a sweep alone can't express "usual for this route," which is why the relative lane learns real baselines instead.

### Alerts

- **`AlertDecision`'s reason is the point, not the boolean.** Four very different quiet mornings (below threshold, immature data, cooling down, stale fare) must not collapse into one `false`.
- **`ImmatureData` is not a weaker `BelowThreshold`.** A day-1 route can score 100 (its only fare is its own min/median/max) — reporting that as "ordinary" would be a lie in the direction that matters.
- **`StaleFare` declines to claim freshness rather than claiming the price rose** — Orbit doesn't know that, only that the evidence is too old to act on.
- **`QuietHours` holds minutes since local midnight only** — no date, timezone or clock — so the wall-clock-to-instant conversion happens exactly once, in `DeliveryWindow`, which is the only timezone-aware line in the pipeline and computes the window's *end* (not a duration) to avoid drifting across DST or a busy queue.
- **The "inside quiet hours" test is `>= start OR < end`**, because the window normally crosses midnight (22:00-08:00); a naive `AND` is wrong in the direction of mailing someone at 3am.
- **`AlertNotification` implements `ShouldQueue` because that's what makes `->delay()` mean anything** — without it, quiet hours silently become decoration.
- **`$alertIds` must stay `protected`, never `private`** — Laravel's `SerializesModels` reflects on parent-class properties and silently drops private ones on the way to the queue, surfacing as a fatal deep inside the delivery listener. This is a DO-NOT-REMOVE landmine.
- **`AlertResource` reads from the stored `payload`, not live relations** — a March alert row must keep quoting March's price and percentage, not be silently rewritten to agree with today's statistics; a deleted rule's past alerts still explain themselves.
- **The digest is deliberately the opposite of an alert**: it ignores cooldown, sensitivity and every interrupt rule, because it's not an interruption — a route suppressed all week by its cooldown still belongs in Sunday's mail.
- **`DigestBuilder` reads only the same classes screens read**, so the mail can never disagree with what a tap-through shows. Routes with no observation are skipped (not scored 0); output is sorted best-first, not watchlist order; rules with 0 matches are omitted rather than shown as "0 matches"; `week()` reads the stored payload, not re-derived data.
- **`RuleMatchNotice` batches every new match into one mail per rule per run** — eleven mails for one rule the morning a sale starts would teach someone to filter this app into a folder they never open. Chips come from stored criteria (`RuleViews`), never re-parsed text, so the mail never claims a filter the owner removed.
- **`MailDealNotifier` is transport-agnostic**; the settings gate (`email_alerts`) lives here so a future push channel doesn't need the deal-decision code to learn who's listening. A refused notice is silent and leaves its ledger row undelivered — an honest record that Orbit decided there was a deal and nobody heard.
- **Quiet hours are a queue delay, not a re-interpretation** — the instant was already settled by `DeliveryWindow`; re-reading it here would be a second chance to lose an hour.
- **An unknown alert-notice type throws rather than being dropped** — an alert that silently goes nowhere is the worst failure this app has, because everything still looks like it's working.

### Pricing

- **`DealScorer` is zero-framework-import PHP**, combining percentile (60), trend (25) and absolute (15) components; missing inputs shrink the *question* (renormalise the weights) rather than the answer (fake a score). Below `minTrackingDays`, the scorer declines outright rather than renormalising — day-1 statistics agree confidently that a route is the cheapest it's ever been, which is the one wrong answer that gets mailed.
- **`noOpinion()` is a distinct method**, not `score(0, null, empty)` — a price of zero would otherwise score 100. Its pill reads "Pricing," not "Watching," because `POST /api/routes/lookup` broke the assumption that reaching this line always meant a watched route.
- **`absoluteComponent()` returns null, not 0 or 100, when a route's floor equals its median** — there's nothing to measure, so the weight falls to components that know something, same as a missing history does.
- **`PriceStats`' five knots must be non-decreasing** — the whole class interpolates between them as a monotone curve, and a p25 above the median would silently run percentiles backwards.
- **`percentileOf()` is piecewise-linear**, the most a five-number summary can honestly claim; a degenerate summary (every knot equal) answers exactly 0.5.
- **`PriceHistory.lastDays()` counts calendar days, not observation points** — counting points would quietly reach further back than asked after a missed poll.
- **`dailyDrift()` uses least-squares slope, normalised by the mean** — first-vs-last would flip on one noisy day; an un-normalised drift means something different on a €40 route than a €400 one.
- **`DatedFare.foundAt` is a third date**, distinct from both the departure date and the fetch date — it exists because Travelpayouts is a cache, and the field lives on the port's own type because it has to survive the trip from adapter to alert policy untouched, never backfilled from `fetched_at`.
- **`NightsBand` is a type, not two nullable ints**, so a reversed or negative pair is caught once at construction; it counts nights, not days (an off-by-one here silently answers the neighbouring question), and 0 is a legal minimum — same-day returns are real, measured fares.
- **`PriceProvider`'s one method matches the calendar endpoint's shape exactly** — the same answer feeds both the heatmap and the daily observation, so there's no second definition of "the current price." `asOf` is deliberately absent: no real API lets you ask about the past (`FakeHistorySeeder` moves the test clock instead).
- **`FakePriceProvider` is not a test double — it's what production runs** until a Travelpayouts key exists; it stamps `foundAt` as "now" rather than leaving it null, specifically so the freshness feature isn't invisible in every sandbox and screenshot.
- **`ReturnFare.return_date` is derived, never stored** (from `nights`), to avoid one fact written twice; `fetched_at` vs `found_at` are not interchangeable, and the gap between them is *wider* here since `/v2/prices/latest` serves a 7-day-deep cache.
- **A duration-band query scope was deliberately not written yet** — writing `whereBetween('nights', ...)` before a screen needs it would mean guessing the shape (per band? per week? per month?) rather than letting the PR with a screen decide.
- **The round-trip test suite is fixture-only**, recorded from the live API on three routes spanning the range (AMS-LIS well-covered, AMS-JFK the long-haul this milestone exists for, EIN-BCN genuinely thin); `Http::preventStrayRequests()` makes "no network" a rule, not an intention.
- **Four request params are each a measured bug if missing**: `one_way=false` (else every return has an empty return_date), `period_type` (the whole horizon in one request), `limit=1000` (default is 30 — AMS-BKK returned 338 vs 30), `currency=eur` (API default is roubles, and the numbers are plausible enough to pass unnoticed).
- **A thin route (23 entries over 14 dates) must not log a warning** — sparse is the normal state of a real route, and a warning per thin route per morning is a log nobody reads.
- **The returns adapter needs its own cache warning key** — sharing the one-way adapter's key would let a failed calendar poll silently swallow the returns warning for fifteen minutes.

### Rules

- **`RuleCriteria`'s six nullable/empty fields all mean "no opinion," never "no results."** An empty `vibes` is "anywhere Orbit knows" — this is what makes chip removal *widen* a rule rather than narrow it to nothing.
- **`from()` must survive JSON an older version of the class wrote** — every field is validated and silently dropped if unreadable, because a rule that throws on load is a screen that can't be opened at all.
- **`MonthWindow` stores months, not dates**, because a rule is a standing alert ("somewhere sunny in spring") that must never expire on an anniversary; wrapping (`to < from`, e.g. winter = Dec-Feb) is the *ordinary* case every method must handle correctly, not an error. `resolve()` deliberately allows a start in the past (asked in April, "spring" is the spring we're standing in) and searches a year back for wrapping windows so a January "winter" rule doesn't miss last December.
- **`RuleVocabulary` is built once in `AppServiceProvider` and injected** — `App\Domain` calls no `config()`, and this is also why the regex parser is a unit test rather than a feature test (no database needed to prove it can read a sentence).
- **`RuleMatchSummary.count` is the true total; the sample is capped for a phone screen.** `pending` marks a count as a floor, not a total — the same sweep can honestly say "2 trips match" before saving and "32 trips match" a minute later.
- **`ParseRuleRequest` is shared by parse and create** — same two fields, and the create screen sends exactly what was last parsed. `removed` chip ids are accepted even when unknown (the client holds them across re-parses of an edited sentence), and `text` is `nullable` next to `present` because Laravel's `ConvertEmptyStringsToNull` turns a cleared textarea into null before any rule runs.
- **`MatchBanner` uses info tone (not "good"), shimmers rather than emptying during a re-parse**, phrases a growing count as a floor ("2 trips match ... so far"), and drops "cheapest €34" from that phrasing — a superlative over a set still being assembled is worse to have said than nothing.
- **`RuleReadingResource` is one shape read by both the create screen and the watch screen** — the same arrangement `RouteSummaryResource` has, for the same reason.

### HTTP layer, requests and resources

- **`RouteCalendarController` answers a month at a time**; an out-of-window month returns an empty grid (`days: []`), not a 404, so paging can walk past the horizon honestly. `month` is validated by regex shape rather than a Carbon parse, because Carbon accepts things like "+3 days" that aren't months.
- **Booking links are sent as *templates*, not 62 per-day URLs** — the day sheet books the date the user tapped, and only the client knows which date that is; named tokens (`{ddmm}`, `{yymmdd}`) keep site-specific date-format knowledge server-side.
- **`UpdateSettingsRequest` is `PUT` with every field `required`**, not `PATCH` — a boolean's absence and its `false` value are indistinguishable once optional, and the failure mode is a quiet-hours toggle that can be switched on but never off. API keys are camelCase (the API's vocabulary), never the database's snake_case.
- **`quietStart`/`quietEnd` are required even when quiet hours are off** — the window is stored either way, so turning the feature back on restores what was chosen rather than resetting to defaults.
- **`sensitivity` validates against the configured levels**, not `between:0,2`, so a fourth sensitivity level is one config entry away.
...
- **`SearchAirportsRequest`'s 2-character floor is a cost decision**: one letter matches roughly a third of 3,270 airports, and ten arbitrary rows back is worse than no suggestions, bought with a round trip. The 60-character ceiling just needs to outrun the longest city name.
- **The route-lookup throttle's two limits (6/min, 20/hour) are the fare-provider budget divided up**, not a round number: one miss costs ~7 provider requests, and the ordinary 06:00 hour already claims ≤176 of the ~200/hour ceiling.
...
- **Both login actions answer JSON, never a redirect** — the caller is `fetch()` from a page that must not navigate; a 302 would be followed and returned as a 200.
...
- **`/login` is a named route purely for its name** — Laravel's guest-redirect middleware resolves `route('login')` eagerly, before deciding whether the response should be JSON, so an unnamed login route turns every unauthenticated request into a 500 rather than a 401.
...
- **`PollFares`/`PollReturns` are commands, not `Schedule::job()` per route** — `routes/console.php` loads on every artisan invocation including `migrate` on an empty database, so enumerating routes there would query in the boot path of every command and fail on the first deploy.
- **`--far` is a flag, not a day-of-week check inside the job** — the job stays a plain "poll this many days," so a retried Saturday job still fetches what it promised; the crontab-shaped truth belongs in the schedule where it reads as one line.
- **`orbit:poll-returns` shipped unscheduled on purpose, then got scheduled anyway** — the polling was already happening from a cron outside the repo, because return-trip history only accumulates in real time; the schedule entry moved that clock into the deployed stack rather than starting a new cost.
- **`orbit:discover` sits in the 20-minute gap between the returns poll's fan-out tail (ends 05:04) and the Monday stats refresh (05:40)** — sequential rather than overlapping in the shared 05:00 hour.
- **`RetainBuilds` keeps 3 builds** (a phone that missed 3 deploys is still worth rescuing, a 4th is not), never prunes by mtime (an unchanged chunk keeps its old, still-correct name), and warns rather than fails on a no-build checkout so it doesn't bury the real error from a failed `npm run build`.
- **Deployment invariants on `docker-compose.yml` are unit-tested** because a stray capability, an exposed port or a drifted uid break nothing at runtime — they're found by someone reading the file, and the cost of the test (thirty lines) is much less than the cost of the miss.

### PWA and service worker

- **The precache list is generated at request time from the live Vite manifest**, not a static file, so the worker and the page can never disagree about what a build actually produced. `globe.gl` and lazy-loaded views are excluded because they're reached through dynamic (not static) imports — precaching 1.9MB of globe on first install would spend mobile data nobody asked to spend yet.
- **The service worker is served by a route, not a static file**, for three reasons: the precache list must be generated fresh, the cache headers must force revalidation (a year-cached worker can never be updated), and the `Service-Worker-Allowed` scope header needs to be set explicitly. It's public (no auth) because the login screen registers it too, and a redirect-to-login served as JavaScript would install a login page as a worker.
- **`pwa.js`'s core assertion is negative: nothing reloads unless the button was pressed** — `controllerchange` fires on its own once a new worker claims the page, and an auto-reload on it would throw someone off the screen they were reading, minutes after a deploy they had no part in.
...
- **`AirportField.vue`'s two-tier suggestion panel (curated, then world) exists because the curated 184 rows answer instantly from memory and are the only ones the rule engine can match**; the `open` flag is owned by the parent form (not the field) because focus/blur races with the suggestion panel made the "Look up" button unpressable while a panel was open.
- **`RouteDetail.vue` distinguishes three separate dates rigorously** — observation date, departure date, and found-at date — because conflating any two of them is exactly the bug that shipped a stale, dead fare in 42-point type. (See §top-routedetail entries, folded into RouteDetail.vue's own pointer comments.)
...
- **`WatchRow.vue`'s switch and bin are structurally outside the `<a>` link** (asserted as DOM structure, not just behaviour) so a tap can never both toggle the route and navigate away.
- **`TabBar.vue`'s centre button became a search icon on 2026-08-16** on the evidence that the first day of real use produced 32 look-ups and 0 rules; the search tab has no stray `aria-label` (unlike the `+` it replaced) because its visible text is now its accessible name.
...
- **`stores/airports.js` is a composable, not a Pinia store** — it was a store for exactly as long as the search screen had one input box; a singleton would have the From field repainting itself with whatever was typed into To.
- **Curated results always sort before world results** in the merged typeahead — those 184 rows are the only ones a rule can ever match, so someone typing "lisb" for Lisbon must see Lisbon first.
...
- **`motion.js` centralises reduced-motion handling because `scroll-behavior` CSS doesn't reach a JS-driven `scrollIntoView({behavior: 'smooth'})`** — the JS argument takes precedence over the stylesheet by specification, so the media query has to be asked again in JavaScript.
- **`heat.js` is a plain module, not a Vue component**, so the grid, legend and day sheet all get the same colour for the same fare by calling the same function; its colour ramp is literal (not from `tokens.css`) because it's a data-visualisation scale that must mean the same thing in both themes, not a themed surface.
...
- **`user_settings` is a separate table from `users`**, keeping Laravel's own authentication table narrow; it enforces one row per account via a unique constraint on `user_id`, and its defaults live in the migration alone (no second copy in a model or seeder to drift from). Quiet hours are stored as wall-clock local time — the one deliberate exception to "everything else is UTC."
- **`deal_rules` stores both `raw_text` and `criteria`, and neither is derivable from the other** — `raw_text` is what the create screen puts back in the textarea on edit; `criteria` is what was understood *after* the owner corrected it by removing chips, and re-parsing the text on load would silently undo that correction.
- **`calendar_fares.found_at` was added because `fetched_at` alone conflated "when Orbit asked" with "when the price was last actually seen"** — nullable, with no backfill (there's no honest value for rows written before the column existed) and deliberately unindexed (nothing sorts or filters on it; the staleness sweep uses `fetched_at`, the right axis for that question).
...
- **`BuildsRouteData::trackedSince()` writes exactly one old observation, not a series** — a multi-day fixture would hand the scorer a computable trend and move every test's score in ways nobody could explain on paper.
...
- **`CandidateScorer` is pure and knows nothing about request cost** — ranking a sweep is the only stage judgeable by reading it; `shortlist()` cuts to a policy-carried number without an opinion on why it's five.
- **`usort()` isn't stable across candidates with equal €/km** — the route code breaks ties so a rerun with identical inputs never reorders itself and looks like news.
- **`shortlist()` dedupes by destination across all three origin sweeps** — Málaga appeared in both the DUS (€29) and EIN (€31) sweeps on 2026-08-16; paying for two window fetches and two Google searches to say one thing wastes two of five slots.
- **`percentile()` is the strictly-cheaper share (0 = cheapest); an empty window scores 100, not 0** — the direction of failure has to be "prove it," not "assume it," or missing data would masquerade as a bargain.
- **Median, not mean, throughout the discovery statistics** (`CandidateScorer`, `RouteBaseline`, `SelfStatsProvider`) — a long right tail (one €600 date in a month of €78s) would inflate every claimed saving.
- **`DealCandidate` sorts by €/km (buys "surprise," not just "near") but isn't sufficient alone** — long-haul fares can land in the same ratio band as genuine bargains, hence `DiscoveryPolicy`'s absolute price ceiling. Unit is millieuros/km throughout; a zero-distance candidate must never reach the scorer (division produces INF, which sorts to the front).
- **A discovery's age (`foundAt`) being null means TOO OLD** — the opposite of `AlertPolicy`'s null-means-fresh, because that column arrived after existing rows needed protecting and this feature has no legacy rows at all.
- **`GoogleVerdict` reads Google's market, never the candidate's own price** — the obvious "candidate < Google's typical low" rule would confirm all three real 2026-08-16 finalists despite a measured 6x price discrepancy. `confirmed` is stored, never re-derived, so a retuned rule can't rewrite what a card already claimed.
- **`RouteBaseline`'s discount is a window median (not average), can't be derived from a sweep** (only one Dublin row exists there), and its sample-day count travels with the median always — `RelativeLanePolicy::admits()` is the only thing allowed to judge whether it's enough. `discountOf()` returns negative unclamped (a ranking key must keep its sign); `savingsOf()` clamps to never-negative (a saving of "€-12" isn't a sentence a card can print).
- **Haversine deliberately does NOT use the provider's `distance` field** — it was once catastrophically wrong (AMS-BRU: 5,951km reported vs. 158km real), which would have silently topped the discovery ranking with a terrible fare. Degrees in throughout (matches `airports.lat/lng`); clamped input avoids `asin()` of a value a few ulps outside [0,1] producing NaN, which would sort a route to the top as an "infinite" deal.
- **`Discover` command is scheduled where `orbit:poll-returns` deliberately wasn't** — it ships with a reader (`GET /api/discoveries`) on day one, and it's safe to schedule since nothing here can send mail; it's one job (a ranking, not a per-route fan-out), behind a command name (not schedule-file code) since `routes/console.php` loads on every artisan invocation including `migrate` on an empty database. An empty run prints "nothing today" honestly rather than falling back to the least-mediocre candidate.
- **`Discovery` model has no `route_id` and no `user_id` by design** — a discovery is an unwatched, unpriced pair (named by `code` string until tapped) and a fact about the world, not an account's relationship to one. Its `live` scope is shared by the API read and the prune's definition of "current" — two spellings would let a row vanish from the screen while the prune still thought it was live. Ordered by €/km, not price (sorted by price it's just the nearest airports). `isVerified` reads the stored verdict only, never recomputed.
- **`discoveries` table is deliberately not history** — the only fare table in the app that doesn't accrue; three timestamps (`found_at` = price age, `discovered_at` = sort key, `departure_date` = flight date) must not be mixed. €/km is stored, not computed on read, because the API sorts on it and a discovery's ranking must keep meaning what it meant when the row was earned. `percentile`/`savings_cents` are nullable — sometimes the honest answer is "we didn't find out." Unique key is `(code, departure_date)`, the upsert key that lets a repeat candidate update rather than duplicate.
- **`discovery_lanes` migration bundles the `lane` column and the `discovery_baselines` table in one migration because they're one feature** — the column is a label nothing can earn without the table, and the table is a measurement nothing reads without the column; rolling back half would leave the job writing a lane it can't compute. Baselines live in their own table, never `calendar_fares` — writing there would mint speculative `routes` rows and leak into the alert pipeline.
- **`DiscoveryResource` deliberately does NOT extend `RouteSummaryResource`** — a discovery has no score, no history, no statistics, and publishing a half-filled route summary would put `score: 0`/`confident: false` on a card whose whole point is "this is different." `lane` is published as a string (matching the enum) since the client draws a different sentence per lane. `verified`/`label` are read off the stored verdict, never re-derived — the same argument `AlertResource` makes.
- **`DiscoveryCard.vue` is a sibling of `SpotlightCard.vue`, deliberately not a copy** — a discovery has no history so it shows its working (age, percentile, savings) instead of a percentage. Badge label/verified come from the server, never composed client-side. Age is always drawn when known (route detail only shows it past a threshold) since a discovery is the least-verified thing in the app. Only relative finds get an explanatory "for this route" sentence; an absolute find's price is already the whole sentence. `.find__from` stays route-pair-only (e2e reads it to navigate); the lane tag is a sibling element, never appended text.

### Alerts (continued)

- **`AlertLedger`'s cooldown read is one query, not one per route**, and fetches only the cooldown window — anything older can't suppress anything, keeping the read constant-size on a table that grows forever. The rule is part of the cooldown key, so two rules matching the same route are two independent cooldowns. `triggered_at` (not `delivered_at`) anchors the cooldown, since quiet hours can put hours between decision and delivery. The digest read is delivered-rows-only, route-or-rule-only — a deal still queued behind quiet hours hasn't been seen yet.
- **`AlertCandidate` has two named constructors and one private one** — a watched route carries a deal score and maturity; a rule match (already price-tested by the matching engine) carries neither, and `null` on those two fields means "no threshold applies," not "scored zero." The price is on both (the cooldown is about the price, not the score). `fareFoundAt`/`departureDate` are on both and never null — they feed the freshness guard every alert goes through. Both dates belong to the cheapest calendar fare, not the daily observation the score came from — the reader will click through to that fare, so freshness is asked of it.
- **`alerts` table backs both the cooldown and the history** — foreign keys are nullable/nullOnDelete (deleting a rule must not erase its alert history); no unique key on `(user, route, type, day)` since the 5%-further-drop rule can legitimately fire twice in one day. `type` is a string column, not a native enum, so adding a kind never needs a migration on a table holding a year of history. `score` is nullable (null on a rule match or digest, since neither has one — a zero would read as "scored terribly"). `channel` is a column, not an assumption, since PLAN.md has web push arriving later and multi-channel needs a per-channel "did this go out" answer.
- **`BuildsAlertData` test fixture is one shared set of price knots** so a route's score is a checkable function of price alone (€44→94, €60→72, €93→40). It writes exactly one price in the scoring window, never a series — two prices would give the scorer a trend to fold in and break the paper-checkable arithmetic. Uses the array mailer, not `Mail::fake()` — a fake never fires `NotificationSent`, the event `MarkAlertsDelivered` stamps `delivered_at` from, so a faked test couldn't tell delivered from undelivered.

### Pricing (continued)

- **`ReturnTripProvider` port is a sibling of `PriceProvider`, not a replacement** — a return trip isn't derivable from a one-way price (a long-haul one-way reads 58-69% of the return fare). Coverage is sparse/uneven by API nature, never a bug; a missing pair means "no fare found," never €0; no `asOf` parameter, same reasoning as `PriceProvider`. The duration band is a filter applied after fetching, not a request parameter — `trip_duration` is silently ignored by the API (verified byte-identical with/without it); null means every stay length, which is what the daily poll wants.
- **`ReturnTrip` value object is `DatedFare`'s sibling** — grain is `(departure, nights)`; nights is the stored fact and the return date is always derived, midnight-anchored so it compares equal to a `DATE` column written elsewhere. Zero nights is legal (same-day returns are real fares); negative is refused at construction.
- **`return_fares` is a separate table, not a nullable `nights` column on `calendar_fares`** — the unique key, the retention rule, and every existing read of `calendar_fares` all differ. `nights` is stored and `return_date` is not (same fact twice would eventually disagree) — nights is the query axis every rule actually asks on and indexes as a plain integer. Unsigned small int, floored at 0. One retention clock, not two (unlike `calendar_fares`, which polls at two speeds) — this table is fetched whole in one request per horizon. `found_at` nullable, meaning "age unknown," never "fresh." Unique key matches the provider's own grain `(depart_date, return_date)`; a second index serves duration-band-first reads, since screens filter on stay length before departure date.
- **`FakeFareModel` is deliberately shared by both fake adapters** so `FakePriceProvider` and `FakeStatsProvider` draw from one distribution — different generators would make every route look like a bargain or a rip-off for no reason a reader could see. Deterministic via crc32, no global random state. Seasonal swing kept to ±14% on purpose — bigger would make every route look permanently 40% off and leave the score no room to distinguish routes. Two 5-day, 38%-off sale windows a year exercise the "insane" score tier deliberately, so it isn't tested only against a polite band around 60.
- **`FakeReturnProvider` is deliberately sparse** (unlike its dense one-way sibling) — real round-trip coverage measured 7.7-33.5% of near-window dates, so a dense fake would build every future screen on an assumption production breaks on day one. Priced as two `FakeFareModel` legs with a discount, sharing the model so one-way and return numbers agree with each other. Always stamped fresh (`foundAt: now`) even though real returns are routinely days old — the one thing this fake deliberately flatters, so the freshness feature stays visible in every screenshot.

### Rules (continued)

- **`RuleMatches` runs four queries for any rule** (destinations, routes, fares, watchlist), each fetched once — a per-route lookup would be 184 queries for "anywhere cheap." Destinations and watchlist are memoised per instance (request-scoped via the container, never stale) and per USER (a bare cache would silently cross-contaminate the moment two users share a request — moot today, deliberate anyway). It creates nothing — an unpriced route just has fewer matches until `SweepRuleFares` runs. Empty criteria finds nothing (unlike empty `vibes`/`origins`, which mean "anywhere") — there's no rule yet, so no results is the honest reading, not a widened one. Cheapest-first with route code as tiebreak, not insertion order.
- **`RuleMatcher` is pure PHP, zero queries**, kept as two independent functions ("where would this send me" / "does this fare qualify") specifically so `SweepRuleFares` can ask the first about routes with no fares yet. Trip length is parsed but deliberately not matched on — `PriceProvider` has no return-leg fact to filter against yet; the chip still shows because the sentence really did say it. Climate filter only runs when a rule names both a warm vibe and a window.
- **`ParsedRule` is one direction only: criteria in, chips out, criteria back** — nothing ever builds chips by hand, which is what guarantees the chips on screen and the saved rule can never describe different trips. `without()` re-derives criteria from remaining chips rather than re-reading edited text. Unknown removed-chip ids are ignored, not rejected — normal while the client re-parses a sentence somebody is still typing.
- **`AnthropicRuleTextParser`: evidence (the owner's sentence) goes in its own content block before the instruction** — the prompt-injection defence, since untrusted text can never become part of the instruction. Schema is enforced server-side (the first text block IS the JSON). `stop_reason` is checked before `content` is indexed — a refusal is a 200 with empty content, and indexing `content[0]` first crashes on exactly the case that most needs to survive. Every failure falls back to the regex parser silently (unlike a comparable analyzer elsewhere that surfaces errors) — there's no useful error to show between two keystrokes. Uses `isset()`, not `?->`, on `stopDetails` — it's uninitialised (not null) when absent, which the null-safe operator doesn't help with.
- **`RuleController`: creating a rule queues a sweep and does not wait for it** — day-1 honesty, since a brand-new rule's honest match count is often zero and fills in a minute later; running the sweep inline would put thirty provider calls behind one tap. A rule with empty criteria is refused as a firehose (matches everything, everywhere), not merely for being blank text. Rule lookups are scoped to the user, never merely filtered by id.
- **`SweepRuleFares` job creates `routes` rows for unwatched pairs on purpose** (a route is a fact about the world, reusable later) and skips already-priced-today codes BEFORE applying the cap, not after — otherwise a rule overlapping the watchlist would never reach its own tail. Its horizon is deliberately shallower than the daily poll's — sweeping a capped 30 routes six months deep would be 210 Travelpayouts requests, over the ~200/hour ceiling on its own.
- **`resources/js/stores/rules.js` shares one store for the create screen's live parse and the watch screen's rule list** because they share a shape and because a newly created rule has to appear in a list the other screen may already be holding — two stores would show a stale count the moment a rule was written. The 500ms debounce lives in `Create.vue` (a textarea concern); the store only refuses stale out-of-order parse responses. `watch()` reuses the existing add-route watchlist endpoint (a rule never adds a route on its own behalf per PLAN.md) and returns the new row so the list can splice it in without a re-fetch.

### HTTP layer, requests and resources (continued)

- **`RoutePairRequest`'s origin is no longer restricted to the three home airports** (removed 2026-08-16, after a day of real use produced 32 lookups and 0 rules) — both ends are now plain `exists:airports,iata`. `config('orbit.origins')` is deliberately untouched and MUST stay that way — it still bounds the nightly rule sweep's budget, and widening this request must never widen a sweep. `size:3` runs before `exists` only to control message order; input is upper-cased before any rule runs (`prepareForValidation`) so the exists check, subclass checks, and the stored row all agree. Two subclasses differ deliberately: `AddWatchedRouteRequest` refuses a duplicate watch, `LookupRouteRequest` refuses nothing.
- **`UpdatePasswordRequest`: `current_password` is the actual security gate, not the session** — a stolen/borrowed phone is signed in too; a session cookie proves device possession, not secret knowledge. `current_password:web` names the guard explicitly rather than trusting the "default" guard, which is whatever middleware last set it. `different:current_password` is what makes this a CHANGE — without it, resubmitting the same password reports success while rotating nothing. Error messages are shown to the person verbatim by the client, so each must name its field.
- **`AirportController`'s search is a `?q=` query (unlike `GET /api/destinations`'s whole-list send)** because 3,270 rows is ~200KB nobody should download before typing. Ten rows returned, merged client-side with the curated list's eight. Deterministic tie-break required — a ranked query with a LIMIT has to be total or the panel reshuffles on re-render. `orderByRaw()` is proven injection-safe by its `literal-string` return type — every value in the CASE expression is a bound `?`.
- **`WatchlistItemController` is deliberately separate from the read-only `WatchlistController`** — the read is the app's tuned launch request; these are one-row taps, and keeping them apart means neither grows the other's concerns. Every write answers in the row's own shape (`WatchlistRouteResource`) so the screen replaces rather than re-fetches. `store()` finds-or-creates (a route is a fact about the world, not a possession — re-using a row hands back price history already paid for) and queues its poll/stats jobs rather than running them synchronously, so a tap gets its row back immediately. `destroy()` preserves the route and its history — only the watchlist row goes.
- **`ManifestController` is a route, not a static file** — chiefly because nginx's stock mime types have no `.webmanifest` entry (its fallback is a download, not a manifest); as a bonus, the name/colours then come from `config/orbit.php`, the same source the meta tag reads, so the two can't drift. Most manifest keys are inert on iOS, this app's only real target — declared anyway for Android/desktop. No `?source=pwa` tracking param, since vue-router would have to carry or strip it forever. Portrait orientation, since the design's camera choreography assumes it. The maskable icon is a separate rendering (not the same file tagged twice) since a maskable crop needs a smaller glyph to survive it.

### Auth and security (continued)

- **`PasswordController`: the session is ROTATED, not ended**, so a phone mid-tap doesn't get kicked off; `logoutOtherDevices()` re-hashes the password (invalidating every other session's stored comparison hash via `AuthenticateSession`) and must run BEFORE the re-login below it, since the recaller cookie the re-login queues must carry the FINAL hash — otherwise this device would sign itself out via its own password change. Answers 200 with a body (not 204) so the screen can render "Password changed" without inferring from a status code, but deliberately not the user object — one refactor away from a hash leak.
- **`AuthenticateSession` middleware must be registered instead of the framework's stock version** — the stock one is fine on its own, but Sanctum rewrites the default guard mid-request, so the stock version would silently check the wrong session's password hash on a password change. The comparison hash is copied (not recomputed) from the sanctum-keyed session slot to the web-keyed one, preserving whatever hash format the framework used.

### Console commands, jobs and scheduling (continued)

- **`ResetHistory`: DO NOT touch the alert ledger or any owner-decided data** (watchlist, rules, settings, routes) — wiping the ledger would let the next real poll re-announce every deal already mailed. `--confirm` (not Laravel's `$this->confirm()`) is required because this runs over `docker compose exec -T`, where stdin isn't a terminal and an interactive prompt would hang or silently proceed on EOF. Deletes by model class, not table name (a renamed table takes the command with it rather than silently truncating nothing); uses DELETE not TRUNCATE (which takes an ACCESS EXCLUSIVE lock, not worth it against a live box); reuses the ordinary poll/stats commands rather than a private copy that would drift, running statistics first since scores are percentiles against them.
- **`RetainBuilds` keeps assets via a per-build ledger, not by file mtime** — a chunk whose content didn't change keeps its old name AND old timestamp across builds, so mtime-based pruning would delete a file the current page still needs. Default `--keep=3` balances phone-cache staleness against build count. A missing manifest is not an error (the deploy runs this right after `npm run build`; a build failure already reported itself). Ledger snapshots use microsecond precision deliberately — two deploys can land in the same second, and second-resolution sorting would tie and let retention drop the wrong build. Scoped to `assets/` only, since `public/build` also holds `manifest.json` and the ledger itself. A kept file also keeps its `.map` (Vite omits maps from the manifest, so a manifest-only prune would delete every sourcemap on its first post-deploy run).
- **`ScheduleTest`: a wrong schedule is invisible** — nothing errors, prices just look a day old, and a missing timezone drifts an hour twice a year unnoticed by anyone. The far poll (04:00 hour) runs in an otherwise-empty hour specifically because months 7-11 cost 12 provider calls/route vs. 7 for the daily near-window poll, and doesn't replace that day's regular poll (both still run). Round-trip polling goes wherever there's room, not beside the other polls — the 06:00 hour is already at 183/~200. Its start time (04:40, not 04:20) is set by the per-minute stagger gap, not the hourly budget — the far poll is still queueing its own fan-out until 04:34, and starting earlier would interleave two bursts in the same minutes. Morning run order (poll, then sweep, then alerts) is load-bearing, not a preference — sweeping before the poll would waste the rule sweep's capped budget re-fetching the watchlist, and alerts running first would mail that morning's verdict against yesterday's prices, invisibly.

### PWA and service worker (continued)

- **`service-worker.js` is served verbatim by `ServiceWorkerController`, not bundled by Vite** — a worker with a content-hashed filename could never be updated (the browser looks for the same URL it registered). Install uses `Promise.allSettled`, not `.all` — one 404 in the precache list must not abort the whole install, since a failed install means no worker at all. `cache: 'reload'` bypasses the HTTP cache during install, avoiding installing an asset the browser cached before this deploy. Activate uses `skipWaiting` + `clients.claim()` rather than waiting for tabs to close, since a home-screen app's tab can stay open for weeks; only prefix-matched caches are ever deleted. Writes are never intercepted — there's no offline queue, and a replayed PATCH would be worse than a failed one.
- **`BuildAssets`/precache list is generated from the live Vite manifest at request time** — the worker and the page can never disagree about what the build actually produced. Fonts precache in woff2 only, not the fallback. A fresh checkout with no build serves the static shell rather than erroring, and an unparseable/truncated manifest degrades the same way rather than 500ing.

### Frontend — globe, tour and geography (continued)

- **`resources/js/lib/geo.js` is pure geometry with zero DOM/canvas knowledge**, which is what lets its numbers (bearing, altitude, great-circle path) be unit-tested in isolation. Angles are degrees at the API boundary (matching the API and globe.gl); bearing is recomputed per animation frame from the CURRENT path segment, not once from the endpoints, since it changes continuously along a great circle. The camera's path midpoint is the true midpoint of the great-circle path, not the naive average of the two lat/lngs — the average is wrong by hemispheres for any route crossing the antimeridian (AMS→NRT would centre on the Atlantic). Longitude interpolates through the shortest delta, not a naive lerp — 179.6°→−179.7° is 0.7° of flying, not 359.3°, or the camera jumps to the far side of the planet for one frame.
- **`resources/js/Components/calendar/month.js`: everything is computed in UTC deliberately** — the API's dates are bare `YYYY-MM-DD` with no zone, and `new Date('2026-06-01')` parses as UTC midnight then answers `getDay()` in the viewer's own timezone, shifting the whole grid a column for anyone west of London. The grid is built from the calendar and filled from the API, never the reverse — `days` arrives with missing dates simply absent, so indexing into it by position would misalign every date after a gap.
- **`e2e/specs/theme.spec.js` reads a real painted CSS property (`color` on `<body>`), not just a resolved custom property** — a custom property resolves whether or not anything actually uses it, so only a painted property proves the theme swap reached the live document. `background-color` is deliberately excluded from that check, since the light theme's `--bg-grad` is a gradient and the shorthand reads transparent in exactly one theme. Search's two boxes are asserted as the most palette-dependent surfaces in the app (card, focus border, a suggestion panel deliberately a different surface from its card, bolded match text) — Home has none of that. Tab labels are asserted here rather than on Home specifically because they were pinned to a test that first rasterises a 1.4MB globe on a software renderer, so on a loaded CI box the label assertion silently never ran.

### Frontend — views and components (continued)

- **`AirportField.vue`: `open`/`close` are requests to the parent form, which decides; a "did-you-mean" guess only appears when nothing matched and the world search isn't still in flight**, and only searches the curated list (never for the pair's excluded end). The box shows what was typed; `code` is the upper-cased form actually submitted — the box no longer shouts every keystroke back in caps. Enter on a box that already holds a known code submits immediately, checked against both curated AND world suggestions (so codes like JFK, outside the curated set, still submit on one Enter). The value watcher fires on the normalised (stripped, not upper-cased) value rather than gating on `open`, since paste/autofill/`fill()` can deliver a value in one event that beats an `if (open)` guard. `choose()` cancels its own just-triggered search on `nextTick`, since a synchronous `clear()` would run before the pre-flush watcher and cancel nothing. The highlight ring has one extra stop beyond the visible rows ("nothing highlighted") so arrowing past the last row returns focus to the box instead of wrapping. The suggestion panel is in the document flow, not floating, because Orbit's 430px column would otherwise let a floating panel sit directly on the buttons beneath it.
- **`WatchRow.vue`'s link stops at the tear line** so the pause switch and remove button are never nested inside the `<a>` (a tap on either would both fire the link and the control). It renders a day-1 route honestly (no data, not a guess). "Paused" outranks the tracking-days note in the stub, deliberately.
- **`boardingPass.js`'s flag swatch and flight number are pure derived functions, never sent by the server** — neither is real data (there is no flight, no airline), and both must be pure so the card doesn't visibly change between renders. Flags are CSS gradients, not images or emoji — an image per country is 28 HTTP requests for 22×15px of decoration, and Windows renders flag emoji as grey letters.
- **`Calendar.test.js`/`Calendar.vue`: which month it opens on is the watched route's cheapest-departure month**, clamped into the eleven-month arrow window and never in the past — it used to always open on the current month, so a route's actual cheapest day could sit several taps away, unmentioned.
- **`Home.vue`'s component `name` is load-bearing** — `App.vue`'s `<KeepAlive>` matches on it to avoid rebuilding the WebGL scene (~2.5MB of texture, an 11-second camera sequence) every time someone glances at another tab and comes back. The watchlist list is the shared store, not fetched independently, so a route paused on another screen doesn't linger in this screen's tour. The greeting deliberately uses the phone's local clock, not the owner's configured timezone — a greeting talks to whoever is holding the phone; everything with a real date uses the server's timezone.

### Stores and client libraries (continued)

- **`stores/airports.test.js`: the world half of the typeahead is a network search, so everything worth testing is a race** — debounce, abort, and specifically the sequence guard, which catches the one case debounce and abort both miss: a request already on the wire when replaced can still resolve after the fact, and without the guard "new" could repaint the panel over "newark"'s stale answer.
- **`stores/discoveries.js` and `stores/watchlist.js` deliberately have no getters** — each screen's own slice of the list (active-only, first code, a count, "the best one") is one line of `computed` where it's needed; a getter here would be a third or fourth opinion about a list that already has legitimate ones per-screen. `discoveries` starts at status `'idle'` (unlike `watchlist`, which starts `'loading'`) since the deals strip is optional and the search screen is fully usable before or without it. Refreshes are deliberately not deduped for either store — each is one GET of a small collection, and a call that quietly declined to fetch would hand its caller a stale list with no way to know. Rows are never cleared before a refresh in either store, so the screen doesn't visibly jump/rebuild on every revisit.
- **`stores/watchlist.js` folds together three screens (globe, watch screen, calendar) that each fetched `/api/watchlist` independently in parallel branches** — three copies that could disagree the moment one of them wrote (pausing a route on the watch screen left the globe still touring it until reload). `add()` throws rather than setting a store-wide `error` (the one place this store differs from `stores/rules.js`) since the add form phrases its own 422 next to the field that caused it.
- **`resources/js/lib/format.test.js`'s `seenLabel` is the single sentence that shows a fare's age everywhere in the app**, so its failure modes are all quiet ones (off-by-one at the hours/days boundary, a fabricated age over an unknown-vintage fare, NaN). A null `foundAt` must never be shown as "just now" — that would be a fabricated claim, worse than showing nothing. Under an hour reads as "just now" rather than a minute count, since nothing in this app moves on a per-minute scale. The hours/days boundary sits at exactly 24 hours, the poll's own period. Day counts round down, not up, since rounding up would overstate a fare's age. A future timestamp (clock skew) reads as "just now," not a negative age.
- **`resources/js/lib/http.js` is the one HTTP client all requests go through**, so "does the server know who I am" has exactly one answer. Cookie authentication only, no tokens — the session cookie is httpOnly and unreadable from JS, so there's nothing in localStorage to steal. `withXSRFToken` re-reads the XSRF cookie per-request rather than a page-load meta tag, since logging in rotates the session and its token. The 401 interceptor is the one place a dead session redirects to login; `/api/me` and `/login` themselves are exempted, since a 401 from either is an expected answer, not evidence of an expired session.

### Tests, seeders and fixtures (continued)

- **`AirportSearchTest`: the seeded tests at the bottom matter most** — everything above proves the SQL against a handful of factory rows; the seeded ones prove that typing "Tokyo" against the real 3,270-row snapshot finds Tokyo (specifically Haneda, not Narita — a fact about the OurAirports snapshot, not this endpoint). `%`/`_` LIKE wildcards are both escaped, the one injection-shaped thing a search endpoint has. This endpoint deliberately does NOT exclude the current origin, unlike `GET /api/destinations` — Amsterdam is a valid destination here since `RoutePairRequest` accepts DUS-AMS; the form itself drops the selected origin.
- **`AnthropicRuleParserTest` is driven through a mock PSR-18 transporter, not a mock of the SDK's `messages->create()`** — the cases worth testing (a refusal, a truncation) are things the SDK itself deserialises, so mocking the SDK's own client would only assert the mock returns what it was told to. `RuleTextParser` implementations must never throw — the create screen asks about half-finished English constantly, so even an unreadable sentence must come back as an answer, never an exception.
- **`BookingLinkTest` is worth its own file because every failure here is silent**: a swapped day/month opens a perfectly working link that searches the wrong date; mismatched IATA casing searches a different place entirely (Travelpayouts' own docs: `ROc1` is Romania business class, `ROC1` is Rochester economy); none of these is a 500 and none would appear in a log. The Aviasales params shape (`AMS1509OPO1`) is verified against Travelpayouts' documentation and live pages, not remembered. No date means the pre-filled search form, not empty results — the honest destination when there's nothing to show yet. The two sites' casings are opposite and both matter (Skyscanner lower-case path, Aviasales upper-case and case-sensitive params). `travelpayouts.marker` rides on the Aviasales hand-off only, and is absent (not sent empty) when unset.
- **`DestinationsApiTest`: 184 curated destinations and 187 airports (184 + 3 origins) are deliberately different numbers**, not a red-test bump — the destinations count moved with world flights (77 European + 107 long-haul), keyed off the `destinations` table rather than `is_origin`, to keep 184 curated places, 3 origins, and 3,083 unrelated OurAirports rows from being conflated.
- **`GoogleFlightsCheckTest`: every price in the discovery funnel is one-way** — comparing against a Google round trip would call a €29 one-way vs €200 return a "discrepancy" that isn't real. `ask()`'s return value distinguishes a billed-but-empty search from one that was never run at all, since callers must know whether SerpAPI's quota was actually spent. A body that doesn't echo EUR, or an unfinished search, is not an answer to the question asked (dollars would read as a bargain; a partial result as a real market).
- **`LivePriceCheckTest`: Google answering without `price_insights` on a thin route still counts as a billed search** — the row is written and the cooldown covers it, since SerpAPI was charged either way; only an unreachable-provider failure spends nothing and leaves the button available to retry. Two taps that race must never produce a 500 — both spend a search, but the row's unique key lets exactly one through.
...
- **`MailRenderTest` exists despite mail being "judged by looking"** because the markdown mailer has silent failure modes that all render valid-looking HTML with the wrong content: `mail.markdown.paths` unset silently swaps in Laravel's stock layout; a theme file's own comments can delete the media-query rule below them (a known cleanup-regex bug); a banner resolved via `asset()` bakes in whatever `APP_URL` was at send time — `http://localhost` in tests, and broken forever once a real mail goes out under the wrong host.
- **`PwaShellTest`: the three PWA-critical routes (manifest, service worker, offline page) break in three ways, none of them loud** — shadowed by the SPA catch-all (registered before them, so without explicit fallback-demotion every one is a 200 of HTML); sessioned (none may sit in the `web` group, or a Set-Cookie per navigation kills edge caching); or simply wrong about the current build. The offline page has no bundle, font, image, or script dependency — each would be unavailable in exactly the conditions offline exists for, and the CSP forbids inline script anyway. Cache-Control is asserted directive-by-directive, since Symfony reorders the header string and comparing it whole would test formatting, not policy.
- **`RuleSweepTest`: the assertions are mostly about budget** — a vibe-less rule sweeps 231 provider calls uncapped, which is the difference between a feature and an outage. Uses `handle()` directly rather than `dispatchSync()` under `Queue::fake()`, since a faked queue records a sync dispatch without running it — asserting on that would pass even if `handle()`'s body were deleted. A rule whose date window names a month past the sweep's shorter horizon still matches on any already-watched route's real fares (`RuleMatches` reads the calendar, not the provider); only *unwatched* speculative fares that far out are missing until the calendar rolls toward them.
- **`WatchlistWritesTest`: origin is no longer restricted to the three home airports here** (inverted 2026-08-16, alongside the search screen) — what stays home-only is the rule engine, which never validated through this request class at all. `is_origin` is a rule-engine-only flag this endpoint has never checked.
- **`WorldFaresTest` exists separately from `TravelpayoutsPollTest`** specifically to check the world-airport claim against the real API before shipping it — every route in the sibling file is AMS-LIS, a well-covered 2,000km short-haul, and doesn't exercise long-haul or obscure pairs at all. Travelpayouts answers AMS-JFK with destination `"NYC"` (a city code) — the adapter must read only `depart_date`/`value`/`actual` from an entry and never trust the echoed origin/destination, or a fare could land on the wrong airport within a city. `data: []` for an unreachable/unusual pair (e.g. EWR-JFK, neither near home) is a real provider answer, not a failure — the test asserts the request reaches the provider rather than dying in validation.
- **`CandidateScorerTest` and `RelativeLaneSelectorTest` use real rows from the actual 2026-08-16 sweep, not invented numbers** — the config thresholds were chosen by looking at this exact data, so a test built from invented numbers would only agree with itself. Defaults are written out rather than read from config, since these are pure unit tests with no framework, and a test reading the file it's checking could never catch a drifted default (`DiscoveryRunTest` is the feature-level guard that config and these hard-coded values still agree). A known route that isn't rare today is dropped from the relative lane, not explored — Orbit already knows what it costs, so spending a fetch to relearn it is exactly what the exploration budget must not do; it becomes explorable again only once its baseline ages out. The exploration rotation's seed includes the day (not just the route) so the same three routes don't explore forever, and deliberately ignores the clock so a lunchtime hand-run of `orbit:discover` reproduces the 05:20 schedule's answer.
- **`RegexRuleTextParserTest`'s first test is the literal contract**: design/README.md's example sentence must produce its six documented chips exactly, since this app ships that sentence pre-typed into the textarea. It loads the real `config/orbit.php` vocabulary, not a test-only one, since the claim being proven is "production's words can read this," not "some regex can." A bare number is never read as a price (trip lengths like "3 nights" share sentences with real prices). Whole-word matching keeps "sun" inside "sunny" (and "mar" inside "market") from accidentally matching the vocabulary.
- **`QuietHoursTest`'s default window (22:00-08:00) is the ordinary case, deliberately the hard one to get right** — a naive `>= start AND < end` isn't merely wrong on a crossing-midnight window, it's wrong in the direction of mailing someone at 3am.

### Dependency injection wiring (`AppServiceProvider`)

- **The three/four fare ports are chosen by config name, and an unknown name throws at resolution rather than silently falling back to a fake** — a production box answering with invented prices because someone mistyped `travelpayots` is this app's single worst failure mode (a real alert about a fare that doesn't exist). Round trips and the origin sweep are each bound as their own switch, even though their real adapters share a vendor/endpoint with another port, because their coverage and failure modes differ and each must be able to fail or be disabled independently.
- **`GoogleFlightsCheck` is bound directly, not chosen by config name**, unlike the fare ports — there's nothing to choose between (SerpAPI is the one thing that exists), so the switch that matters is simply whether the key is present. It's bound fresh, not as a singleton, since it holds no state between calls and a config change during a test should be visible without a container flush. An empty `SERPAPI_KEY` string is read as unset, the same convention `seed.password` uses.
- **Discovery's funnel numbers convert euros to cents at this config boundary**, since everything below HTTP is integer cents; `max_eur_per_km` is the one to double-check (×100 turns €/km into cents/km). The relative lane's `min_discount` is the one number in that lane needing no conversion — it's a fraction, not a euro figure. The €15 minimum-savings floor is shared between the absolute and relative lanes by being passed the same config value, not independently declared, so retuning one doesn't silently diverge from the other.
- **`minTrackingDays` is passed to both `DealScorer`/`ScoringPolicy` and `AlertPolicy` from the same config key on purpose** — "young enough that we won't mail about it" and "young enough that we won't put a verdict on it" have to be one decision, or the screen and the alert engine could disagree about the same morning.
- **`RuleVocabulary` is built once and bound as a singleton**, read by both parsers and `RuleViews`, since `App\Domain` is pure PHP and may not call `config()` itself.
- **`DealNotifier` is bound directly (not by config name), like `GoogleFlightsCheck`** — mail isn't one of two ways to send an alert, it's the only one that exists yet; when push arrives it will be an addition composed alongside mail, not a config-selected alternative.
- **The real Travelpayouts adapters (price, returns, sweep) each throw from their own constructor when the token is missing** — `ORBIT_PRICE_PROVIDER=travelpayouts` on a box with no token must fail loudly at the first poll, not serve an app with a calendar that's quietly, permanently empty. They share the vendor's connection settings (base URL, token, timeouts, retries, warning interval) but keep their own behaviour settings (e.g. `returns.max_nights`, sweep's `limit`) separate, since duplicating the connection half would mean a token rotation that half-worked. The sweep adapter's `limit` deliberately reads from `orbit.returns.limit`, not a separate `discovery.limit` key — it's the same parameter on the same endpoint whose undocumented default (30) once silently discarded 91% of a route's fares, and a second copy is a second place to drift.
...
- **Every rate limiter in this file is keyed on account, not IP, except login** — one account, several devices, one phone whose IP changes mid-sentence. Login is the exception (keyed on email AND IP, 5/min) since it's the app's entire brute-force surface with no account to key on yet, and the email is lower-cased first so `Ghie@`/`ghie@` aren't two buckets. The password-change throttle uses the same 5/min number as login for the same underlying guess, but keys on the account (the caller is always authenticated). The rule-parser throttle (20/min) exists before it's needed — today's regex parser could take any volume, but the day an Anthropic key lands the same route becomes a metered request per keystroke, and nobody will remember to add a limiter that day. The airport-search throttle (60/min) is unusually generous on purpose — it guards a client bug (a debounce that stopped debouncing), not a cost, and a limiter a person can trip by using the app normally is worse than the bug it prevents. The route-lookup throttle's two numbers (6/min burst, 20/hour ceiling) are the fare-provider budget divided up: one miss costs ~7 provider requests, and the ordinary 06:00 hour already claims ≤176 of the ~200/hour Travelpayouts allows.
...
### `routes/web.php` — routing decisions
...
- **`POST /login` and `PUT /api/profile/password` both answer JSON, never a redirect** — the caller is `fetch()` from a page that must not navigate, and a 302 would be followed and handed back as the shell's HTML with a 200 (i.e., read as a successful login).
- **The password-change route sits in the `auth` group (session guard), not `auth:sanctum`, beside `/logout`** — it's a session action (the credential is the cookie, the check is against the session guard's user), so one guard end-to-end keeps it legible; it's also the one authenticated route that checks a secret, hence the only write route that's throttled.
- **`GET /api/me` uses `auth:sanctum` in the `web` group specifically** — Sanctum's guard tries the session guard before looking for a token, so a first-party cookie authenticates with no token anywhere, and being in the `web` group boots the session unconditionally rather than via Sanctum's Origin/Referer heuristic (which can only ever turn an already-signed-in user into a spurious 401 on a single-origin SPA).
...
- **`GET /rules` is a read that computes at request time rather than serving a cached count** — a cached match count would be wrong from the next poll onward.
- **`GET /destinations` sends the whole 184-row list with no `?q=` search** — the client fetches once and filters as someone types, since the payload is small and the alternative buys nothing.
- **`GET /airports` is the one read-group endpoint that's throttled** — it costs no third-party money (unlike the parser/lookup limiters), but it's the one endpoint a keystroke can fire directly, and 60/minute is chosen as "a debounce that has failed" rather than "a person typing."
- **`GET /alerts` has no screen reading it yet, deliberately** — the alerts screen stays settings-only in this PR; the endpoint exists because the mail pipeline is otherwise invisible from outside the database and needs somewhere to answer "did it fire, and did it go out."
- **`GET /discoveries` is a read and deliberately NOT throttled**, unlike everything else upstream of it in the discovery feature — by the time anyone GETs it, it's one indexed query over ~10 precomputed rows; the expensive work already happened at 05:20. No `{code}` and no parameters, since the whole set is the answer.
- **The write-endpoint group sits in the `web` group specifically because that's what gives it CSRF protection** — this app has no `routes/api.php` at all in part because Laravel's `api` group has no CSRF middleware (a token-authenticated client wouldn't need it), and these endpoints are called by a browser carrying a session cookie, exactly the case CSRF exists for. `resources/js/lib/http.js` sends the XSRF header on every request, so the protection costs the client nothing.
- **`POST /routes/lookup` is a write (not a `?refresh` flag on the GET), throttled, and takes no `{code}` in the path** — it can create a route row and spend six or seven metered provider calls, which a browser prefetch or link preview could trigger unprompted on a GET; the pair arrives in the body as `origin`/`destination` so both this and `POST /watchlist` validate through the same `RoutePairRequest` and answer with the same sentences.
- **`PUT /settings` (not PATCH) sends the whole preferences object every time** — see `UpdateSettingsRequest` for why an optional boolean would be a switch that can't be turned off.
- **`POST /rules/parse` is a POST that writes nothing, deliberately** — a GET would put the owner's free-text rule sentence in every access log and browser history between the phone and the server. It's the only throttled route in this file besides login, since the create screen calls it on every keystroke's 500ms debounce and the day an Anthropic key lands, each of those becomes a metered third-party request.
...
- **Three distinct dates are never allowed to substitute for each other**: `history[].date` is when Orbit LOOKED, `cheapest.date` is when you'd FLY, `cheapest.foundAt` is when the price was actually FOUND (a third date, since Orbit's fares are a cache of other people's searches and the headline number can be days old — the origin story of "€36 shown against a live €56"). The page's headline `departure` is sourced only from `cheapest.date`, never derived from `history[].date`. `lastChecked` deliberately doesn't reuse the `departureLabel` formatter even though it would print the same three words, since one names a day-you-fly and the other a day-you-looked.
- **This screen owns an on-demand fetch for a route Orbit has never priced**, since "look before you watch" can land here with a pair that has no route row at all: on a 404 or stale fares it POSTs a lookup and adopts the answer. Three rules bound it: a watched route is never refreshed from here (its staleness is a poll to fix, not a screen's job); the wait is shown and bounded by its own timeout (25s, several times a healthy fetch's 2-3s, and nothing is lost by giving up since the writes are upserts); and what's already on screen survives a failed refresh rather than being replaced by an error page.
...
- **"Seen…" only appears once the cheapest fare is over 24 hours old** — the day sheet prints an age under every price because that's all it has room for; this screen already has a headline, a gauge, a chart, a callout, and a button, and a "Seen 2 hours ago" on a route polled this morning would just be noise that teaches the reader to skip the line where it matters. The 24-hour threshold is deliberately the same number the server uses to decide whether to spend a lookup provider call (`orbit.lookup.fresh_for_hours`), though the two are separate decisions that happen to agree.
- **`justWatched` is a separate flag from `meta.watched`** — a route that was already watched when the screen opened says nothing about it; this flag is the answer to a button pressed a second ago, and a button that vanishes silently is a button people press twice.
- **`pctBelow`'s sign is flipped in the caption sentence** (−14 means 14% ABOVE usual) so the text reads naturally rather than printing "−14% below." The percentage is silent under a live (Google-checked) headline, since the caption is Orbit's opinion of Orbit's own fare and would otherwise read as an opinion about Google's number.
- **`confident: false` suppresses the percentage comparison, not the usual-price line** — with under a week of a route's own statistics, the current price literally IS the median, so stating "36% below its usual €99" would be placeholder arithmetic read out as a finding while the gauge beside it says it has nothing to say. The usual price stays, since it's a fact; only the derived comparison is dropped.
- **An on-demand refresh only fires when a route is BOTH stale AND unwatched** — a watchlist route's staleness is a broken poll to fix, not something worth spending a provider call on from someone's phone; an unwatched route looked up months ago has nothing else that will ever refresh it.
- **A 404 from the read triggers an on-demand lookup rather than an error state** — no route row is the ordinary state of a pair someone just typed into the search screen, so the natural next question is whether it can be priced, not whether something broke.
...
- **`watchRoute()` reuses the exact same store write the add form makes**, through the same store, which is what keeps the globe's tour and the watch list in step with a route added from this screen (`Home` is kept alive between navigations and wouldn't otherwise learn about it until reload).
- **`goBack()` checks vue-router's `history.state.back` before calling `router.back()`** — without the check, someone who opened a shared link straight into a route detail page would be walked out of the app entirely and back to whatever they were reading before it.
- **The "checking" state is visually distinct from the ordinary loading skeleton** — a skeleton says "this is arriving," but what's actually happening is a fare provider being asked six or seven questions across six months, which takes a few seconds and is worth saying out loud (`role="status"`, progress rather than alarm) rather than looking like a broken pulsing page.
- **The watchlist strip sits between the header and the price, only on an unwatched route** — under the header (a fact about the route, "Orbit is not tracking this") but above the price, deliberately not competing with the Book button at the bottom, the one conclusion this screen is allowed to draw.
...
- **The booking CTA's button variant must never add conditions of its own beyond the server's tone** — a callout saying "wait" over a glowing "book" button is the page arguing with itself, and the button wins that argument by being louder; the tone is the server's own judgement (it already accounts for a possibly-gone fare and a live-price contradiction), so the client only renders it.
