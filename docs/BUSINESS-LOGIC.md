...
| stagger | 6 minutes between per-route jobs | `orbit.poll.stagger_minutes` | `App\Console\Commands\PollFares` |
| hourly allowance | ~200 provider requests per IP | `orbit.travelpayouts.hourly_request_limit` | `App\Application\Pricing\FareRequestBudget` |
...
**The request budget**, which is what the stagger is arranged around —
Travelpayouts allows ~200 requests an hour per IP, thirteen routes are watched,
and every job is charged to the clock hour its `index × 6` delay lands it in.
Saturday, the only day all four provider runs share:

| clock hour | what lands there | requests |
| --- | --- | --- |
| 04:00 | far poll routes 0-8 (9 × 12), returns poll routes 0-3 (4 × 1) | 112 |
| 05:00 | far poll routes 9-12 (4 × 12), returns poll routes 4-12 (9 × 1), discovery (59) | 116 |
| 06:00 | poll routes 0-8 (9 × 7), rule sweep (30 × ≤4 months) | **183** |
| 07:00 | poll routes 9-12 (4 × 7) | 28 |

Every figure above is **Saturday's**, the worst day; on an ordinary morning the
04:00 and 05:00 rows are 4 and 68. So the eleven months cost **nothing in the
worst hour**, and neither does the tail of the morning poll: at a six-minute
stagger only nine routes can reach the 06:00 hour, however long the watchlist
gets. What breaches first is **nineteen watched routes**, and not in this table
— every clock hour is still under there, while the sixty minutes from 05:46
cost 206 of ~200 (§27).
`App\Application\Pricing\FareRequestBudget` counts the live watchlist and the
live rules against both readings of the hour, and against a second limit
entirely — whether the last route is polled before `orbit:alerts` reads the
table (§13). All of it runs every morning inside `orbit:poll-fares`, and again
on every watchlist add, rule write and pause-or-resume, and each sentence says
which limit it is at `ERROR`.
...
| --- | --- | --- |
| **06:10 daily** | `orbit:poll-fares` | before the owner is awake, after the airlines' overnight fare loads have settled. Fans out per-route jobs at a 6-minute stagger, each asking for the **near** window; only the first nine reach this clock hour, the rest land in 07:00 (§27) |
| **Sat 04:10** | `orbit:poll-fares --far` | the same fan-out asking for the whole **eleven-month** horizon — twelve provider calls a route where the daily poll costs seven. In its own clock hour because 13 × 12 beside the sweep's 120 would be 276 against a ~200/hour limit; Saturday because eleven months out is holiday planning. It does **not** replace that day's 06:10 poll, and cannot: both write the same near-window observation, idempotently |
...
| **07:10 daily** | `orbit:refresh-return-stats` | round trips, and **daily** where `orbit:refresh-stats` is weekly: this run *writes* the morning's price per duration band into `return_price_history`, and a morning missed is a hole nothing can fill in afterwards (§15). Past 04:40's fan-out as the watchlist grows — at twenty routes its last job is dispatched at 06:34 — and out of the 06:00 hour, which is already at 183 of ~200 (§27). **The last count that clears it is 25**: at a six-minute stagger the returns fan-out's last dispatch is 04:40 + 6(N−1), which reaches 07:10 itself at 26, where 25 puts it at 07:04 — six minutes ahead, against the ~31s worst case of a one-request job (one call, a 15s timeout plus one retry). It calls no provider, so it adds nothing to that count |
...
docker compose exec app php artisan orbit:refresh-return-stats --now
...
`orbit:poll-returns` runs **daily at 04:40** (§13), in the 04:00 hour beside the
far poll rather than in the 06:00 one, which is at 183 of ~200 before it arrives.
It is staggered on the same setting the one-way poll uses, so at thirteen routes
only four of its calls land in the 04:00 hour and the other nine fall into 05:00
(§27). **04:40 rather than 04:20** is the per-minute limit rather than the hourly
one: the two fan-outs must not start on the same minute.
...
### What a return trip costs now, and what it usually costs

`orbit:refresh-return-stats` · `App\Domain\Pricing\ReturnBandPrice` ·
`App\Application\Pricing\ReturnBandPrices` · `return_price_history` and
`return_price_stats`.

A one-way fare has one price per departure date, so "the current price" is the
cheapest of them (§6). A round trip has a price per *pair* of dates, and most
(route, departure date) pairs here carry exactly one stay length — so "the
cheapest 7-night trip leaving on 3 November" usually has no answer at all. The
grain that does have one is the **duration band**, which is also the axis a
person asks along ("a week away"), and the one `tripLengthNights` was written
against.

| rule | what |
| --- | --- |
| **R1** | The grain is **route × duration band** (`orbit.returns.durations`, inclusive at both ends). A fare belongs to a band when its `nights` falls inside it; a fare in no band is counted nowhere. |
| **R2** | The **current price** is the cheapest in-band fare Orbit still holds: departures from today to `orbit.returns.stats.window_days` ahead, and only rows a poll has seen within `orbit.returns.stale_after_days`. A tie goes to the fare found most recently, then to the shorter stay. |
| **R3** | A current price carries the **stay length and find time of the fare that won**. A band is a range, and this endpoint serves a seven-day-deep cache; `found_at` null means "not known", never "found this morning". |
| **R4** | The **usual price** is the five-number summary (`PriceStats`, nearest-rank) of *the same pool* — same route, same band, same window, same staleness — so today's number is drawn from the days it is compared against. |
| **R5** | Below `orbit.returns.stats.min_samples` in-band fares there is a current price and **no usual price**: `null`, never an invented distribution. |
...
| what | value | config key |
| --- | --- | --- |
| departures considered | 334 days | `orbit.returns.stats.window_days` |
...
somebody was really quoted. Sparse is the *normal* state here — a measured route
carried two rows — and the honest answer for such a route is a price with no
verdict attached, which is the same answer §7's day-1 floor gives a young route.

**Staleness is measured on `fetched_at`, not on `found_at`.** The first says the
last poll still saw the fare; the second says how old the fare was when it was
found, and on this endpoint it is routinely days old (§15, above). Filtering on
`found_at` would discard more than half of an already thin table and would answer
a question nobody asked here; R3 carries that age forward instead, so the PR that
alerts on these fares can apply `orbit.alerts.max_fare_age_days` itself.

**Nothing reads `return_price_history` yet**, deliberately — the same argument
that put the poll on the schedule ahead of its first reader. A longitudinal view
only accumulates in real time, and §6's blend cannot be checked against real data
until a month of mornings exists; the PR that first *scores* a round trip is the
...
afterwards. It calls no provider, so it costs the request budget nothing.
...
**The budget, which is the real constraint** — Travelpayouts allows ~200 requests an hour per IP. A fan-out job is charged to the clock hour its `index × 6` delay lands it in, so the table is by hour rather than by run (thirteen watched routes and one active rule, today's; Saturday, the only day all four runs share):

| clock hour | what lands there | cost |
| --- | --- | --- |
| 04:00 | far poll routes 0-8 (9 × 12 = 108), returns routes 0-3 (4 × 1) | **112 of ~200** |
| 05:00 | far poll routes 9-12 (48), returns routes 4-12 (9), discovery (3 + 35 + 21 = 59) | **116 of ~200** |
| 06:00 | poll routes 0-8 (9 × 7 = 63), sweep 30 capped × ≤4 months (§34) | **183 of ~200** |
| 07:00 | poll routes 9-12 (4 × 7) | **28 of ~200** |

Lane B's 21 requests buy two things: the fares it surfaces, and the baselines it remembers — a lane-B fetch that produces no card still writes a `discovery_baselines` row (§30). Discovery spends no SerpAPI beyond the ≤5 searches shared across both lanes, absolute taking priority. Monday's 05:40 `orbit:refresh-stats` shares the 05:00 hour and costs nothing here — `self` reads Orbit's own tables. Every other morning the same table reads 4 / 68 / 183 / 28, the far poll being weekly.

Where it breaks, so nobody has to rediscover it. The stagger is what bounds each hour: a run starting at :10 can put at most nine of its jobs in its own clock hour at six minutes, so the daily poll contributes 63 to the 06:00 hour beside the sweep's 120 however long the watchlist gets. What grows instead is the 05:00 hour, which collects both earlier fan-outs' tails, and then the 06:00 hour again from W = 15, when the returns fan-out reaches it too. **A clock hour is not the only way to read an hourly limit**, and it is the more forgiving one: at W = 19 every clock hour is under (189 at 05:00) while the sixty minutes from 05:46 cost **206**. The first breach is therefore at **W = 19** on the rolling window, and would have been W = 20 on hour buckets alone. Nothing here is remembered: `App\Application\Pricing\FareRequestBudget` builds this table from `orbit.poll.stagger_minutes`, `orbit.rules.sweep_cap`, `orbit.discovery.shortlist` and the live counts, and logs at `ERROR` when **either** measure crosses `orbit.travelpayouts.hourly_request_limit`. It checks the alert run's clearance in the same breath and reports it as a separate sentence, because the two have opposite fixes.
...
`stagger_minutes = 6` spaces the per-route jobs so thirteen routes' worth of provider calls don't arrive as a burst — the real APIs are rate-limited per minute too — and, since 2026-08-30, so that a fan-out's tail falls into the *next* clock hour, where the headroom is. Three minutes kept all thirteen routes inside one hour, which is exactly what put that hour at 211 of ~200. What six minutes costs: the 06:40 sweep starts with seven routes still queued rather than two, and `orbit:alerts` moved from 06:55 to 07:35 to keep seeing the whole morning (§13). 07:35 clears the fan-out to **fourteen** watched routes: the last poll is dispatched at 06:10 + 6(W-1) and needs four more minutes (seven near months, each a 15s timeout and one retry, rounded up), so W = 14 finishes 07:32 and W = 15 finishes 07:38. **This limit binds long before the budget's nineteen**, and the 08:00 quiet-hours ceiling means the alert run cannot simply keep moving — widening the stagger again buys request headroom and spends this. `FareRequestBudget` warns on it separately, in its own sentence.
...
`stats.window_days = 334` is which departures the summary considers, and it is `window_days`'s number written out for the third time in this file — what is **kept** and what is **comparable** are two decisions that happen to agree, the arrangement `selfstats.cross_section_days` has for the same reason, and `tests/Feature/ReturnStatsRefreshTest` is the drift guard. It is deliberately NOT `selfstats.cross_section_days`'s 181: that cap exists because `calendar_fares` is polled at two speeds and the far months are cache-thinned into a seasonal-peak sample, where `return_fares` is fetched whole in one request, and halving an already sparse table would cost more than the skew is measured to be worth. Reverse it when the far half of a return route is measured and found to be peak-heavy — see `docs/DECISIONS.md`: the-round-trip-window-is-the-whole-table.

`stats.min_samples = 5` is how many in-band fares there must be before a *usual* price is claimed at all; below it a band has a current price and a `null` summary (§15 R5). Five because a five-number summary of four fares repeats one of them, and because the honest answer for a route with two rows is a price with no verdict attached — the same answer the day-1 floor gives a young one-way route (§7).
...
- **The route-lookup throttle's two limits (6/min, 20/hour) are the fare-provider budget divided up**, not a round number: one miss costs ~7 provider requests, and the ordinary 06:00 hour already claims ≤183 of the ~200/hour ceiling (§27). `POST /routes/lookup` is a write (not a `?refresh` flag on the GET) and takes no `{code}` in the path — it can create a route row and spend six or seven metered provider calls, which a browser prefetch or link preview could trigger unprompted on a GET; the pair arrives in the body so this and `POST /watchlist` validate through the same `RoutePairRequest`.
...
| what a round trip costs now and usually | `app/Domain/Pricing/ReturnBandPrice.php`, `app/Application/Pricing/ReturnBandPrices.php` |
