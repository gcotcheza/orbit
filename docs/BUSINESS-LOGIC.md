...
So the eleven months cost **nothing in the worst hour**, and neither does the
tail of the morning poll: at a six-minute stagger only nine routes can reach the
06:00 hour, however long the watchlist gets. What breaches first is still the
ordinary morning, now at **twenty watched routes** (201 of ~200), because the
returns fan-out starts spilling into it as well.
`App\Application\Pricing\FareRequestBudget` counts the live watchlist and the
live rules against this table, and against a second limit — whether the last
route is polled before `orbit:alerts` reads the table (§13). Both run every
morning inside `orbit:poll-fares` and again on every watchlist add, and each
says which limit it is at `ERROR`.
...
Where it breaks, so nobody has to rediscover it. The stagger is what bounds each hour: a run starting at :10 can put at most nine of its jobs in its own clock hour at six minutes, so the daily poll contributes 63 to the 06:00 hour beside the sweep's 120 however long the watchlist gets. What grows instead is the 05:00 hour, which collects both earlier fan-outs' tails, and then the 06:00 hour again from W = 15, when the returns fan-out reaches it too. The first breach is at **W = 20** (201 in the 06:00 hour). Nothing here is remembered: `App\Application\Pricing\FareRequestBudget` builds this table from `orbit.poll.stagger_minutes`, `orbit.rules.sweep_cap`, `orbit.discovery.shortlist` and the live counts, and logs at `ERROR` when the peak crosses `orbit.travelpayouts.hourly_request_limit`. It checks the alert run's clearance in the same breath and reports it as a separate sentence, because the two have opposite fixes.
