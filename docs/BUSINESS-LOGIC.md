...
- **`DealRules` takes a `notice` prop, and `/create` passes `false`.** `stores/rules.js` has one `error` ref for two different failures: the live parse of the sentence being typed, and the load of the saved list. On the create screen both would have printed the same sentence twice — once under the textarea that was refused, once beside the rules in the master. The rule adopted is that the screen owning the write owns the sentence about it, so the create screen keeps its own `.error` line and the section beside it stays quiet. The list's own "Could not load your rules — Try again" state is untouched by the prop, because that one is about the list rather than about a write.
