...
- **`vite.config.js` refuses to let the build empty `public/build`** if Vite's manifest comes back suspiciously small — a build that silently produced nothing would deploy a site with no CSS or JS behind a green pipeline, discovered only by loading the page. The ESM bundler build is named explicitly rather than left to the default, and `src=""` rewriting is handled for the handful of tags Vue's compiler touches, both because guessing wrong here fails silently at asset-serve time, not at build time.
