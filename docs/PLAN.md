...
**From PR3 onwards, `scripts/check.sh` must pass before a PR is merged.** It runs Pint, `composer audit`, PHPStan (level 8, no baseline), `npm audit`, ESLint, Vitest and PHPUnit inside the containers, in that order, and stops at the first failure. A finding is fixed or ignored with a reason next to it — this project has no baseline for new debt to hide in.
