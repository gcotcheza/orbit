...
final class GateRunnersTest extends TestCase
{
    ...
    public function only_the_overlay_step_is_a_step_a_runner_can_skip(): void
    {
        ...
        $this->assertSame(
            ...
            'Every step but the overlay must run in both runners; only the overlay itself is '
            .'conditional. A check that moved inside this branch stopped running for the developer, '
            .'or a further step was added outside it and now runs twice in overlay mode.'
        );
        $this->assertCount(
            count(self::ORDER) - 1,
